...
$('.delete-my-events').on('click', function()
{
  ...
  $('.sonu-button-'+event_id).button('loading');
  $.confirm({
					...
					buttons: {
						'confirm': {
							...
							action: function () {
								...
								{
									$('.sonu-button-'+event_id).button('reset');
...
/* Expire My Listing */	
	...
	{
		  var event_id = $( this ).attr("data-myevent-id");
		  $('.sonu-button1-'+event_id).button('loading');
			$.confirm({
				title: get_strings.confirmation,
				icon: 'fa fa-question-circle',
				theme: 'supervan',
				...
				buttons: {
					'confirm': {
						text: get_strings.ok, 
						action: function () {
							$.post(ajax_url,	{action : 'expire_my_events', event_id:event_id}).done( function(response) 
							{
								$('.sonu-button1-'+event_id).button('reset');
								var get_r	=	response.split( '|');
								if( $.trim(get_r[0]) == '1' )
								{
	...
	{
		  var event_id = $( this ).attr("data-myevent-id");
		  $('.sonu-buttonz-'+event_id).button('loading');
			$.confirm({
				title: get_strings.confirmation,
				icon: 'fa fa-question-circle',
				theme: 'supervan',
				...
				buttons: {
					'confirm': {
						...
						action: function () {
							...
							{
								$('.sonu-buttonz-'+event_id).button('reset');
								var get_r	=	response.split( '|');
								if( $.trim(get_r[0]) == '1' )
								{
									$.alert({title:get_strings.congratulations,rtl:yes_rtl,icon:'fa fa-smile-o',theme:'modern',animation:'scale',type:'green',content:get_r[1],buttons: {okay:{btnClass:'btn-blue'}}});
